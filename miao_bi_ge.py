...
# 小说主地址
req_url_base = 'https://www.miaobige.com/'
headers = {
...
def get_txt(txt_id):
    ...
    try:
        ...
        # 获取最近更新章节名称
        lately = soups.select('meta[property="og:novel:latest_chapter_name"]')
        ...
        print("正在寻找第一章页面。。。")
        # 进入循环，写入每章内容
        chapter_list = soups.select('#readerlists li a')
        for chapter in chapter_list:
            try:
                href = chapter['href']
                req_chapter = urllib.request.Request(req_url_base + str(href), headers=headers)
